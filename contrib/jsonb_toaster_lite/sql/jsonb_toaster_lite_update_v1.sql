...
-- ============================================================
-- T5  DIFF-on-DIFF rebase (v2a)
--     First eligible same-length update on row 1 entered DIFF mode
--     (after T1).  A SECOND same-length update on the same DIFF row
--     should now REBASE: emit a fresh DIFF over the SAME embedded
--     base, discarding the old inline diff.  No DIFF stacking.
-- ============================================================
...
-- Expect calls +1 (hook fires), diffs +1 (rebase emitted a fresh
-- DIFF over the same base; old inline diff discarded).
...
-- ============================================================
-- T9  rebase trajectory: 3 consecutive eligible updates on the
--     same key produce 3 DIFF emissions, no fallback.
-- ============================================================

-- Fresh row 6 to isolate from earlier state.
INSERT INTO crit_u1 SELECT 6, jsonb_build_object(
    'id',       42,
    'status',   'STATUS_active',
    'kind',     'KIND_invoice',
    'tenant',   'TENANT_acme',
...
UPDATE crit_u1 SET jb = jsonb_set(jb, '{status}', '"STATUS_step1"'::jsonb) WHERE id = 6;
UPDATE crit_u1 SET jb = jsonb_set(jb, '{status}', '"STATUS_step2"'::jsonb) WHERE id = 6;
UPDATE crit_u1 SET jb = jsonb_set(jb, '{status}', '"STATUS_step3"'::jsonb) WHERE id = 6;

SELECT jbtl_update_calls() AS calls_t9,
       jbtl_update_diffs_emitted() AS diffs_t9;
-- Expect calls = 3, diffs = 3 (POINTER -> DIFF -> DIFF rebase -> DIFF rebase).

SELECT id, jb -> 'status' AS final_status FROM crit_u1 WHERE id = 6;

-- ============================================================
-- T10  DELETE after rebased DIFF + VACUUM
--      Removes the row that has been rebased multiple times.
--      Confirms the embedded base chunks are cleaned correctly
--      via tsr_delete on the DIFF varlena.
-- ============================================================
DELETE FROM crit_u1 WHERE id = 6;

VACUUM crit_u1;

-- Other rows still readable and unchanged from u1_post fingerprints.
SELECT id,
       md5(jb::text) = (SELECT post_md5 FROM u1_post p WHERE p.id = c.id)
       AS preserved_after_t10
  FROM crit_u1 c
 ORDER BY id;
