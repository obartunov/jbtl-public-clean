...
--	T2  DELETE + VACUUM
--	T3  aborted UPDATE
--	T4  VACUUM FULL / CLUSTER refusal + recovery via SUBTREE off
...
-- Deterministic starting state: suppress NOTICE for "already exists".
...
-- ============================================================
-- T4  VACUUM FULL — REFUSED on SUBTREE-managed table
--
-- Scenario:
--   Several rows with SUBTREE.  VACUUM FULL.
--
-- What is being pinned (M9.1-fix behaviour):
--   - VACUUM FULL is REFUSED with ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE
--     while jbtl_subtree_refs has any edge for the target's
--     reltoastrelid;
--   - the refusal aborts cleanly, no orphan introduced;
--   - logical value remains readable after the refused attempt
--     (no corruption);
--   - CLUSTER is refused similarly;
--   - after dropping SUBTREE rows and gc, VACUUM FULL is permitted
--     and preserves logical value across the rewrite.
-- ============================================================
...
-- Pre-state: 3 edges (one per row).
SELECT 'pre_attempt' AS phase,
       (SELECT count(*) FROM jbtl_subtree_refs r, pg_class c
        WHERE c.relname='t4_st' AND r.parent_toastrelid = c.reltoastrelid)
           AS edges_for_t4,
       jbtl_subtree_refs_check() AS dead;

-- Assertion 1: VACUUM FULL refused (SQLSTATE 55000).
DO $$
BEGIN
    VACUUM FULL t4_st;
    RAISE NOTICE 't4_vfull_unexpectedly_succeeded';
EXCEPTION WHEN object_not_in_prerequisite_state THEN
    RAISE NOTICE 't4_vfull_refused_as_expected';
END;
$$;

-- Assertion 2: CLUSTER refused symmetrically.
CREATE INDEX t4_id_idx ON t4_st (id);
DO $$
BEGIN
    CLUSTER t4_st USING t4_id_idx;
    RAISE NOTICE 't4_cluster_unexpectedly_succeeded';
EXCEPTION WHEN object_not_in_prerequisite_state THEN
    RAISE NOTICE 't4_cluster_refused_as_expected';
END;
$$;

-- Assertion 3: catalog unchanged after refusals.  Each refusal
-- aborted its own VACUUM/CLUSTER transaction; the catalog is
-- exactly as before.
SELECT 'after_refused_attempts' AS phase,
       (SELECT count(*) FROM jbtl_subtree_refs r, pg_class c
        WHERE c.relname='t4_st' AND r.parent_toastrelid = c.reltoastrelid)
           AS edges_for_t4,
       jbtl_subtree_refs_check() AS dead;

-- Assertion 4: logical value still readable (no corruption).
SELECT id,
...
-- Recovery: force a re-toast through the writer.  An identity
-- UPDATE (SET col = col) does NOT work because the SUBTREE
-- varlena passes through unchanged; casting to text and back
-- yields a plain in-memory jsonb that the writer routes to
-- POINTER mode (GUC is off).
SET jsonb_toaster_lite.enable_subtree_storage = off;
UPDATE t4_st SET jb = jb::text::jsonb;
...
-- Assertion 5: VACUUM FULL now succeeds, preserves logical value.
VACUUM FULL t4_st;

SELECT id,
       md5(jb::text) = (SELECT md5_pre FROM t4_pre p WHERE p.id = c.id)
       AS t4_md5_preserved_after_vfull
  FROM t4_st c
 ORDER BY id;
