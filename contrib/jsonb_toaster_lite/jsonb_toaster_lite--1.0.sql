...
--
-- Test-only helper exposing the L1.2c-1 sliced-read counters
...
--
-- L1.4: KVMap-aware top-level object field lookup with sliced TOAST.
--
...
--                                   not yet thread it through L1.4)
...
--
-- jbtl_update_probe(j, key, new_value)
--   For a JBTL_POINTER-stored jsonb document, predicts what an
--   L2.1 same-length top-level scalar update would do *without*
--   modifying storage.  Surfaces the locator + validation
--   decisions and the predicted physical-rewrite footprint.
--
--   Output columns:
--     can_fast_update       bool     true iff every L2.1 acceptance
--                                    constraint is met (POINTER mode,
--                                    key in fast-path universe, same
--                                    type, same encoded length)
--     fallback_reason       text     'ok' on success; explanatory
--                                    string on every other path
--     value_offset          int      offset of the value bytes in
--                                    the raw jsonb body (0 on fail)
--     old_len               int      encoded length of the old value
--     new_len               int      encoded length of the new value
--     chunks_total          int      total chunks of the value
--     chunks_rewritten      int      chunks the L2.1 update would
--                                    overwrite (overlap count)
--     chunks_untouched      int      chunks_total - chunks_rewritten
--     write_fraction        float8   chunks_rewritten / chunks_total
--     rewrite_amplification int      bytes_written / bytes_changed
--                                    per chunk; 1 on success
--
CREATE FUNCTION jbtl_update_probe(j jsonb, key text, new_value jsonb,
                                  OUT can_fast_update bool,
                                  OUT fallback_reason text,
                                  OUT value_offset int,
                                  OUT old_len int,
                                  OUT new_len int,
                                  OUT chunks_total int,
                                  OUT chunks_rewritten int,
                                  OUT chunks_untouched int,
                                  OUT write_fraction float8,
                                  OUT rewrite_amplification int)
RETURNS record
AS 'MODULE_PATHNAME', 'jbtl_update_probe'
LANGUAGE C STRICT IMMUTABLE PARALLEL SAFE;

--
-- L14 layout-map probe (debug helper).
--
-- jbtl_l14_layout_map(j) — for an L14-shape jsonb (root object with
-- key1/key2/key3/key4, where key2/key4 are flat arrays), reports
-- per-path physical locality classification.  Fixed scope, fixed
-- paths.  No path parser.  Body in jsonb_toaster_lite_layout_map.c.
--
-- Output columns:
--   path                   the path in display form (key2[5000] etc.)
--   kind                   "scalar" / "object" / "array" / "missing"
--   byte_offset            absolute offset of the value's bytes in
--                          the body
--   byte_length            encoded length of the value
--   chunk_lo, chunk_hi     contiguous chunk range covering the value
--   distinct_pages         distinct toast pages the chunk range spans
--   parent_metadata_pages  pages required to reach this path through
--                          structural metadata (root header + JEntries
--                          + KVMap + key area + nested array header
--                          if applicable)
--   fragmentation_ratio    distinct_pages / minimum_compact_pages
--                          (1.0 = optimally packed, >1.0 = smeared)
--   class                  inline / compact_range / large_extent /
--                          needs_staged / fallback
--
CREATE FUNCTION jbtl_l14_layout_map(j jsonb,
                                    OUT path text,
                                    OUT kind text,
                                    OUT byte_offset int,
                                    OUT byte_length int,
                                    OUT chunk_lo int,
                                    OUT chunk_hi int,
                                    OUT distinct_pages int,
                                    OUT parent_metadata_pages int,
                                    OUT fragmentation_ratio float8,
                                    OUT class text)
RETURNS SETOF record
AS 'MODULE_PATHNAME', 'jbtl_l14_layout_map'
LANGUAGE C STRICT;

-- β bridge proof helpers (debug-only, used by regression).
...
-- L2.1b: counter of DIFF pointers actually emitted by jbtl_update
...
-- M5.0a: test fixture for synthesising JBTL_POINTER_SUBTREE values
...
--   in a later milestone.  Per @yoda M5.0 spec section 17.
--
CREATE FUNCTION jbtl_subtree_refs_gc() RETURNS int
AS 'MODULE_PATHNAME', 'jbtl_subtree_refs_gc' LANGUAGE C;

--
-- M5.0b-2 test helper: allocate one parent_valueid via the production
...
--
-- M5.0b-2 test helper: check parent_valueid uniqueness probe.
--
