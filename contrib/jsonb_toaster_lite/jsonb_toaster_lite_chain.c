 ...
 *	  Chunk machinery for jsonb_toaster_lite (writer side, L1.2a).
 *
 * Source-of-truth provenance:
 *	  Ported from postgrespro/postgres@jsonb_toaster (HEAD 583a292),
 *	  contrib/jsonb_toaster/jsonb_toast_internals.c, lines:
 *	      484..621   jsonx_toast_write_slice
 *	      623..819   jsonx_toast_save_datum_ext
 *	      821..833   jsonx_toast_save_datum
 *	      835..904   jsonx_toast_delete_datum
 ...
 *
 *	  This commit (L1.2a) ports the writer side only.  Reader-side
 *	  functions (fetch iterator, sliced detoast, diff applier) are
 *	  deferred to L1.2b/L1.2c.  Per @yoda's split: do not bring
 *	  fetch_iterator, sliced detoast, diff applier, direct-TID
 *	  compression, or chunked arrays into this commit.
 *
 *	  GSON-grep verified clean on the source range 484..904 prior to
 *	  porting (0 occurrences of any forbidden generic-jsonb dispatch
 *	  identifier from K1's discipline list).
 *
 ...
 *
 *	chunk_tids:
 *	    If non-NULL, the caller has pre-allocated an array of
 *	    ItemPointerData[nchunks] which will be filled with the t_self
 *	    of each toast row inserted.  Used by DIRECT_TIDS mode (caller
 *	    later wraps the value with jbtl_toast_make_pointer_with_tids).
 *
 *	compress_chunks:
					   ...
					   ItemPointerData *chunk_tids, bool compress_chunks)
{
	...
	while (slice_length > 0)
	{
		...
		{
			/*
			 * Per-chunk pglz compression (L1.2c-2).
			 *
						  ...
						  bool compress_chunks)
{
	...
	{
		compress_chunks = false;
		*p_chunk_tids_ptr = jbtl_toast_make_pointer_with_tids(toasterid, &toast_pointer, data_todo, &chunk_tids);
	}
 ...
 *	store path used by the L1.2b acceptance test.
 */
Datum
 ...
 *	Plain-chunk full-read counterpart of jbtl_toast_save_datum (L1.2b).
 *
 ...
 *	reads, and diff overlays.  L1.2b does not need any of that.
 *
 ...
 *
 *	When L1.2c lands per-chunk decompression and sliced read, this
 ...
 *	declared but unused at runtime; that is acceptable for L1.2b.
 *
							...
							int32 *out_pages_touched)
{
	...
	/*
	 * Refuse to handle compressed externals here.  L1.2b's sole job is
 ...
 *	Real sliced read for plain chunks (L1.2c-1).
 *
 ...
 *	requested byte interval, which is exactly what L1.2c-1 acceptance asks
 ...
 *	across slice calls; that is L1.2c-2 work and lands together with the
 ...
 *	Focused per-chunk-compressed reader (L1.2c-2; cleanup-2).
 *
 *	NOT a port of jsonx_fetch_datum_iterate.  That function is a
 *	multiplexer over plain / compressed_chunks / direct-TIDs /
 *	compressed-TIDs / diff modes through a state machine.  L1.2c-2's
 ...
 *	[startbyte, endbyte]:
 *	  - cleanup-2 removes a second pass that re-opened the toast
 ...
 *	    sizes the L1.2c-2 path targets (toast values up to a few
								   ...
								   int32 *out_bytes_decompressed)
{
	 ...
	 * unwrap.  (Was M8 in the post-L1.2c-2 review.)
	 */
	Assert(!VARATT_EXTERNAL_IS_COMPRESSED(*toast_pointer));
