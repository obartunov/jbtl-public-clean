...
toast_custom_datum_size(const void *ptr, ToastPtrSizeType sz_type)
{
	...
	toasterid = VARATT_CUSTOM_GET_TOASTERID(ptr);
	...
	if (routine->tsr_size != NULL)
	{
		Size	size = routine->tsr_size(ptr, sz_type);
...
detoast_external_attr(varlena *attr)
{
	 ...
	 * (see toast_custom_datum_size for the contract).
	 */
	if (VARATT_IS_CUSTOM(attr))
	{
		...
		if (get_toaster_routine_for_id_hook == NULL)
			ereport(ERROR,
					(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					 errmsg("custom TOAST pointer encountered but no "
		...
		if (routine == NULL)
			ereport(ERROR,
					(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					 errmsg("custom TOAST pointer references unknown "
							"toaster with OID %u", toasterid)));
...
toast_raw_datum_size(Datum value)
{
	...
	{
		result = toast_custom_datum_size(attr, TPTR_RAW_SIZE);
	}
	else if (VARATT_IS_COMPRESSED(attr))
	{
...
toast_datum_size(Datum value)
{
	...
	{
		result = toast_custom_datum_size(attr, TPTR_STORAGE_SIZE);
	}
	else if (VARATT_IS_SHORT(attr))
	{
