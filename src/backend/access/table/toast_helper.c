		...
		else
		{
						...
						ttc->ttc_am_options)) != (Datum) 0)
			{
				if (new_value_after_copy != ttc->ttc_values[i])
				{
					...
					ttc->ttc_values[i] = new_value_after_copy;
					ttc->ttc_attr[i].tai_colflags |= TOAST_NEEDS_FREE;
					ttc->ttc_flags |= (TOAST_NEEDS_CHANGE | TOAST_NEEDS_FREE);

					new_value = (struct varlena *) DatumGetPointer(new_value_after_copy);
				}
 ...
 */
void
toast_tuple_externalize(ToastTupleContext *ttc, int attribute, int maxDataLen, uint32 options)
{
	Datum	   *value = &ttc->ttc_values[attribute];
